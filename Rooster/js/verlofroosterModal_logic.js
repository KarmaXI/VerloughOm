 ...
 */
/**
 * Opent de modal voor het aanvragen van verlof.
 * Haalt de formulierinhoud van 'pages/meldingVerlof.aspx',
 * initialiseert het formulier en stelt de submit actie in.
 *
 * @async
 * @function openVerlofAanvraagModal
 * @param {Date} geselecteerdeDatum - De initieel geselecteerde datum voor de verlofaanvraag.
 * @param {Object} medewerkergegevens - Object met gegevens van de medewerker (bijv. { Username: 'domein\\user', Naam: 'Jan Jansen', Id: 123, Email: '...' }).
 * 'Username' hier is idealiter de genormaliseerde username of de volledige loginNaam.
 * 'Naam' of 'Title' wordt gebruikt voor weergave.
 * 'Id' is het SharePoint User ID.
 * 'loginNaam' (indien aanwezig) is de volledige claims-based UPN.
 */
async function openVerlofAanvraagModal(geselecteerdeDatum, medewerkergegevens) {
    ...
    if (!medewerkergegevens || !(medewerkergegevens.Username || medewerkergegevens.loginNaam) || !(medewerkergegevens.Naam || medewerkergegevens.Title)) {
        ...
        if (typeof toonToast === 'function') {
            toonToast('Fout: Medewerkerinformatie is onvolledig. Kan verlofaanvraag niet starten.', 'error');
        } else {
            alert('Fout: Medewerkerinformatie is onvolledig.');
        }
        return;
    }

    const teGebruikenUsernameVoorContext = medewerkergegevens.Username || medewerkergegevens.loginNaam;
    const teGebruikenDisplayNaam = medewerkergegevens.Naam || medewerkergegevens.Title;

    window.verlofModalMedewerkerContext = {
        gebruikersnaam: teGebruikenUsernameVoorContext,
        displayName: teGebruikenDisplayNaam,
        id: medewerkergegevens.Id,
        email: medewerkergegevens.Email,
        loginNaam: medewerkergegevens.loginNaam
    };
    console.log(`[${functieNaam}] Context voor verlofmodal ingesteld:`, window.verlofModalMedewerkerContext);

    const basisSiteUrl = "https://som.org.om.local/sites/MulderT/CustomPW/Verlof/cpw/Rooster/";
    ...
    try {
        ...
        if (!response.ok) {
            const errorText = await response.text();
            console.error(`[${functieNaam}] Fout bij ophalen van ${modalFormUrl}: ${response.status} ${response.statusText}. Response: ${errorText.substring(0, 500)}`);
            if (typeof toonToast === 'function') toonToast(`Kon formulier niet laden: ${response.statusText}`, 'error');
            return;
        }
        const rawHtml = await response.text();
        console.log(`[${functieNaam}] Modal content succesvol opgehaald.`);
        ...
        if (!formElement) {
            console.error(`[${functieNaam}] Kon <form id="verlof-form"> niet vinden in de opgehaalde HTML van ${modalFormUrl}.`);
            if (typeof toonToast === 'function') toonToast('Formulierstructuur onjuist.', 'error');
            return;
        }
        console.log(`[${functieNaam}] <form id="verlof-form"> gevonden.`);

        const formHeaderToRemove = formElement.querySelector('.form-header');
        if (formHeaderToRemove) {
            formHeaderToRemove.remove();
            console.log(`[${functieNaam}] Eigen .form-header van verlof formulier verwijderd.`);
        }

        const formActionsToRemove = formElement.querySelector('.form-actions');
        if (formActionsToRemove) {
            formActionsToRemove.remove();
            console.log(`[${functieNaam}] Eigen .form-actions van verlof formulier verwijderd.`);
        }

        let notificationArea = formElement.querySelector('#modal-notification-area');
        if (!notificationArea) {
            ...
            if (formElement.firstChild) {
                ...
                formElement.appendChild(notificationArea);
            }
            console.log(`[${functieNaam}] #modal-notification-area toegevoegd aan formulier.`);
        }

        // CORRECTIE HIER: Gebruik outerHTML om de <form> tag zelf mee te nemen.
        const modalContentHtml = formElement.outerHTML;
        console.log(`[${functieNaam}] Modal content HTML voorbereid (met outerHTML).`);
        ...
        const modalConfig = {
            ...
            actionButtonCallback: async () => {
                console.log(`[${functieNaam}] Actieknop 'Aanvraag Indienen' geklikt.`);
                if (typeof window.handleVerlofModalFormSubmit === 'function') {
                    const success = await window.handleVerlofModalFormSubmit();
                    if (success) {
                        console.log(`[${functieNaam}] Verlof formulier succesvol verwerkt, modal wordt gesloten.`);
                        if (typeof closeModal === 'function') closeModal();
                        if (typeof laadInitiëleData === 'function') {
                            laadInitiëleData(true);
                        }
                    } else {
                        console.log(`[${functieNaam}] Verwerking verlof formulier mislukt, modal blijft open.`);
                    }
                } else {
                    console.error(`[${functieNaam}] Functie window.handleVerlofModalFormSubmit niet gevonden.`);
                    if (typeof toonToast === 'function') toonToast('Fout bij verwerken formulier.', 'error');
                }
            },
            customClasses: 'max-w-2xl',
            onOpen: () => {
                console.log(`[${functieNaam}] Modal geopend. Initialiseren formulier logica...`);
                if (typeof window.initializeVerlofZiekMeldingModal === 'function' && window.verlofModalMedewerkerContext) {
                    window.initializeVerlofZiekMeldingModal('verlofAanvraag', window.verlofModalMedewerkerContext);
                    console.log(`[${functieNaam} - onOpen] initializeVerlofZiekMeldingModal aangeroepen met type 'verlofAanvraag'.`);
                } else {
                     console.warn(`[${functieNaam} - onOpen] initializeVerlofZiekMeldingModal of verlofModalMedewerkerContext niet beschikbaar.`);
                }

                const medewerkerDisplayField = document.getElementById('MedewerkerDisplay');
                if (medewerkerDisplayField && window.verlofModalMedewerkerContext) {
                    medewerkerDisplayField.value = window.verlofModalMedewerkerContext.displayName || 'Niet beschikbaar';
                    console.log(`[${functieNaam} - onOpen] MedewerkerDisplay veld ingesteld op: ${medewerkerDisplayField.value}`);
                } else {
                    console.warn(`[${functieNaam} - onOpen] MedewerkerDisplay veld of verlofModalMedewerkerContext niet beschikbaar.`);
                }

                const redenSelect = document.getElementById('Reden');
                if (redenSelect && typeof laadSelectOpties === 'function') {
                    laadSelectOpties("Verlofredenen").then(opties => {
                        redenSelect.innerHTML = '<option value="">-- Kies een reden --</option>';
                        opties.forEach(optie => {
                            const optionElement = document.createElement('option');
                            optionElement.value = optie.Id;
                            optionElement.textContent = optie.Title || optie.Naam;
                            redenSelect.appendChild(optionElement);
                        });
                        console.log(`[${functieNaam} - onOpen] Verlofredenen dropdown gevuld.`);
                    }).catch(error => {
                        console.error(`[${functieNaam} - onOpen] Fout bij laden verlofredenen:`, error);
                        if (redenSelect) redenSelect.innerHTML = '<option value="">Laden mislukt</option>';
                    });
                } else {
                     console.warn(`[${functieNaam} - onOpen] "Reden" select element of laadSelectOpties functie niet beschikbaar.`);
                }

                const startDatePicker = document.getElementById('StartDatePicker');
                const endDatePicker = document.getElementById('EndDatePicker');
                if (startDatePicker && endDatePicker && geselecteerdeDatum instanceof Date) {
                    const lokaleDatum = new Date(geselecteerdeDatum.getTime() - (geselecteerdeDatum.getTimezoneOffset() * 60000 ));
                    const isoDate = lokaleDatum.toISOString().split('T')[0];
                    startDatePicker.value = isoDate;
                    endDatePicker.value = isoDate;
                    console.log(`[${functieNaam} - onOpen] Start- en Einddatum ingesteld op: ${isoDate}`);
                } else {
                    console.warn(`[${functieNaam} - onOpen] Datum pickers of geselecteerdeDatum niet correct beschikbaar.`);
                }
            }
        };
        ...
        if (typeof openModal === 'function') {
            ...
            if (window.domRefsLogic && window.domRefsLogic.modalContent) {
                ...
                console.log(`[${functieNaam}] Class 'verlof-modal-body' toegevoegd aan modalContent.`);
            }
            ...
            console.error(`[${functieNaam}] Generieke openModal functie niet gevonden.`);
            if (typeof toonToast === 'function') toonToast('Interne fout: Modal kan niet geopend worden.', 'error');
        }

    } catch (error) {
        console.error(`[${functieNaam}] Algemene fout bij openen verlofaanvraag modal:`, error);
        if (typeof toonToast === 'function') toonToast('Er is een fout opgetreden bij het laden van het formulier.', 'error');
    }
}
